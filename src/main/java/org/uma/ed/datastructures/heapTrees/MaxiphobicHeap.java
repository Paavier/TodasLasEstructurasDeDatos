package org.uma.ed.datastructures.heapTrees;
...
public class MaxiphobicHeap<T> implements Heap<T> {
  ...
  private static <T> MaxiphobicHeap<T> merge(Comparator<T> comparator, ArrayList<Node<T>> nodes) {
    if (nodes.isEmpty()){
      return MaxiphobicHeap.empty(comparator);
    }

    MaxiphobicHeap<T> heap = MaxiphobicHeap.empty(comparator);
  ...
  private static <T> Node<T> copyOf(Node<T> node) {
    ...
    Node<T> newNode = new Node<>(node.element, node.weight,copyOf(node.left),copyOf(node.right));
    return newNode;
  }
  ...
  private Node<T> merge(Node<T> node1, Node<T> node2) {
    ...
    if(weight(node1.left) >= weight(node1.right) && weight(node1.left) >= weight(node2)){
      biggest = node1.left;
      merged = merge(node1.right, node2);
    }else if(weight(node1.right) >= weight(node1.left) && weight(node1.right) >= weight(node2)){
      biggest = node1.right;
      merged = merge(node1.left, node2);
    }else{
      biggest = node2;
      merged = merge(node1.left, node1.right);
    }
