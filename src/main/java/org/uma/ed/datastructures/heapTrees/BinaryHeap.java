...
public class BinaryHeap<T> implements Heap<T> {
  ...
  public static <T> BinaryHeap<T> copyOf(BinaryHeap<T> that) {
    return BinaryHeap.from(that.comparator, Arrays.asList(that.elements));
  }
  ...
  public void insert(T element) {
    ...
    this.size++; // importante el orden
    heapifyUp(this.size - 1);
  }

  // Checks if elements[index1] < elements[index2]
  private boolean lessThan(int index1, int index2) {
    return comparator.compare(this.elements[index1], this.elements[index2]) < 0;
  }
  ...
  private boolean isNode(int index) {
    return index >= ROOT_INDEX && index < this.size;
  }
  ...
  public T minimum() {
    ...
    return this.elements[ROOT_INDEX];
  }
