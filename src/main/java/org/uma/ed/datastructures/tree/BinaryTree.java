...
public class BinaryTree {
  ...
  public static int maximum(Node<Integer> root, Comparator<Integer> comparator) {
    ...
    if(root.right != null){
      int rightMax = maximum(root.right, comparator);
      if(comparator.compare(rightMax, max) > 0){
        max = rightMax;
      }
    }

    if(root.left != null){
      int leftMax = maximum(root.left, comparator);
      if(comparator.compare(leftMax, max) > 0){
        max = leftMax;
      }
    }
  ...
  public static int count(Node<Integer> root, int element) {
    ...
    return occurrences + count(root.right, element) + count(root.left, element);
  }
